...
NioApp = function (NioApp, $, window, document) {
    ...
    // Languages
    // It's inportant to use lower case
    NioApp.langs = {'en':{'name':'En', 'flag':'flag-icon flag-icon-um'}, 'ru':{'name':'Ру', 'flag':'flag-icon flag-icon-ru'}};
	...
	NioApp.Util = {};
	// Exchange Rate
	NioApp.Util.exchange = async function() {
	  	return new Promise((resolve, reject)=>{

		   	let requestURL = 'https://min-api.cryptocompare.com/data/price?fsym=BNB&tsyms=USD,ILS,JPY,EUR,WAN,RUB,GBP ';
			...
			request.onload = function() {
				_exchange = request.response;

				NioApp.Util.numberOfMembers();
			  	//$("#currentExchangeRate")[0].innerHTML = _exchange.USD.toFixed(2).replace(/(?!^)(?=(?:\d{3})+(?:\.|$))/gm, ' ');
    ...
    NioApp.components.docReady.push(NioApp.Util.exchange);

    // Language change
    NioApp.Util.changeLang = function(_locale) {
    ...
    // Number of Members
	NioApp.Util.numberOfMembers = async function() {
	  	return new Promise((resolve, reject)=>{

		   	let requestURL = window.location.protocol+'/data';
			...
			request.onload = function() {
				...
				if (data.members*1>1000) {

					$('#currentMembersCount').show();

				} else {

					$('#currentMembersCount').hide();
				}

			  	$("#members")[0].innerHTML = data.members.toFixed(0).replace(/(?!^)(?=(?:\d{3})+(?:\.|$))/gm, ' ');
			  	if (data.progress != undefined)
			  	$("#basicIncomeProgress").attr('style', 'width:'+data.progress+'%;');
			  	$("#basisIncomeSumm")[0].innerHTML = data.summ.toFixed(4);
			  	$("#currentExchangeRate")[0].innerHTML = (data.summ*_exchange.USD).toFixed(2).replace(/(?!^)(?=(?:\d{3})+(?:\.|$))/gm, ' ');
				resolve (true);
			}
	...
	NioApp.Util.chekReferralLink = function (){
	...
	// Get cookies value 
	NioApp.Util.getValueFromCookies = function(_value, _prefix, _notNumber) {
	...
	// ClassInit !Util @v1.0
	NioApp.Util.classInit = async function() {
		...
		let temp = NioApp.Util.getValueFromCookies('lang', 'cryptolife', true);
			
		// Check for null and zero values
		if (temp!= null && temp !=0) {
			NioApp.Util.changeLang(temp);  	
		} else {
			NioApp.Util.changeLang("en");
		}			
		...
		if (window.ethereum) {
			...
			let mainChainId = '0x61'; // '0x539',

			// Request account access
		    let accounts = await window.ethereum.request({ method: 'eth_requestAccounts' });

		  	try {
			    $.getJSON("../contracts/CryptoLife.json", async function (data) {
				    ...
				    // Deploy the contract
			      	let instance = await contract.deployed()
			   		...
			   		// Check registration in the contract
			   		let isRegistered = await instance.isRegistered({from:accounts[0]});

			   		if (isRegistered) {

			   			$(".regButton").each(function(){this.innerHTML = l100n.localize_string("i-accButton")}).attr("href","account.html").addClass("accButton");

			   		} else {

			   			NioApp.Util.chekReferralLink();
			   		}
    ...
    // Calculator
    NioApp.Util.calculator = function (){
    	var $steps = $(".btn-step"),
    		$cost = $("#calc-sum"),
    		$summ = $("#calc-total"),
    		$buttons = $(".btn-level"),
    	...
    	if ($steps.exists()){
    		...
    		$(".btn-step.btn-light").on("click", function (){

    			$steps.each(function(k){
	    			if (k>0 && !$(this).hasClass("btn-outline")){
    		...
    		$(".btn-step.btn-info").on("click", function (){
    			$(this).removeClass("btn-outline");
    			$steps.each(function(k){
	    			if (k>1 && !$(this).hasClass("btn-outline")){
	    				$(this).addClass("btn-outline");
	    			} 
    			});
    			changButtons("info");
    		...
    		$(".btn-step.btn-primary").on("click", function (){
    			$steps.each(function(k){
	    			if ($(this).hasClass("btn-outline")){
	    				$(this).removeClass("btn-outline");
	    			} 
    			});
    			changButtons("primary");
		...
		function changButtons(step){
			$buttons.each(function(){
				$(this).removeClass("btn-light").removeClass("btn-info").removeClass("btn-primary").addClass("btn-"+step);
			});
			$currentStep = step;
			calculate();
		}
		...
		function calculate(){
			$cost[0].innerHTML = $currentButton.attr($currentStep+"-lprice").toString().replace(/(?!^)(?=(?:\d{3})+(?:\.|$))/gm, ' ');
			$summ[0].innerHTML = ($currentButton.attr($currentStep+"-lprice")*6*0.75).toString().replace(/(?!^)(?=(?:\d{3})+(?:\.|$))/gm, ' ');
			
			$(".current-step").each(function(){
				if ($currentStep == "light") {
					this.innerHTML = "Light"
					
				} else {
					if ($currentStep == "info") {
						this.innerHTML = "Smart";
					} else {
						this.innerHTML = "Prime";
					}
				}
				$(this).removeClass("text-light").removeClass("text-info").removeClass("text-primary").addClass("text-"+$currentStep);
			});

			$(".current-level").each(function(){this.innerHTML = $currentButton[0].innerHTML});
		}

    };
	NioApp.components.winLoad.push(NioApp.Util.calculator);
