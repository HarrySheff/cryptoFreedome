App = {
  ...
  loadPartnerData: async function (toRender) {
	...
	readPastEvents: async function (){
			...
			App.renderNotifications();
				...
				// Change title
				$(".page-content > h5")[0].innerHTML = '<span id="m-title-2">'+l100n.localize_string("m-title-2")+'</span>';
				$(".page-content > h5").addClass('text-uppercase');
	...
	eventHandling: async function (_event, _needRender){
		...
		return new Promise(async function(resolve, reject){
		  ...
		  // Is it common partner's event
			...
			} else if(_event.event == 'registration' && _event.args.sponsorAddress.toLowerCase() == App.account) {
  ...
  renderRefferalLinks: function(){
  	...
  	function reffealAction(_number){
  		switch(_number){
  			...
  			break;
  			case 1: 
  				navigator.clipboard.writeText(window.location.host+'/register.html?ref='+App.partnerId);
		   		Lobibox.notify('success', {
						pauseDelayOnHover: false,
						icon: 'bx bx-copy',
						continueDelayOnInactiveTab: false,
  ...
  renderNotifications: function () {
  	...
  	let newsCount = 0;
  	let notificationsList = $("#email-list");
  		...
  		if (App.notifications[i].new) {
	  		...
	  		let messDate = new Date(App.notifications[i].dateTime).toLocaleString();
	   		...
	   		if (App.notifications[i].new){
	   			...
	   			temp = '<span class="badge badge-success">New</span> ';
	   		}

	   		mess.find(".msg-info")[0].innerHTML = temp+App.notifications[i].info.replace(/<br>/g, "  ");
  ...
  setDefaults: function (){
  	...
  	$("#userId")[0].innerHTML = '<span class="badge badge-danger">Wallet not registered</span>';
		...
		$(".email-list").each(function(){
				$(this).css("height",  $(window).height()-$(this).offset().top-$("#footer").height()-parseInt($(".email-wrapper").css("margin-bottom").replace("px","")) - parseInt($("#footer").css("padding-bottom").replace("px","")) -  parseInt($("#footer").css("padding-top").replace("px","")));
		});
